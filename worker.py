...
def verify_higgsfield_read_only():
    """Verify Higgsfield through the provider adapter without paid generation."""
    provider = HiggsfieldProvider()
    print(f"Higgsfield API configured: {provider.configured}", flush=True)
    if not provider.configured:
        print("HIGGSFIELD VERIFY: skipped - credentials missing", flush=True)
        return
    try:
        authenticated, status_code = provider.verify()
        if authenticated:
            print("HIGGSFIELD VERIFY: SUCCESS", flush=True)
            print("HIGGSFIELD VERIFY MODE: provider-adapter non-generation probe", flush=True)
        elif status_code == 401:
            print("HIGGSFIELD VERIFY: FAILED", flush=True)
            print("HIGGSFIELD ERROR: invalid or unauthorized API credentials", flush=True)
        else:
            print(f"HIGGSFIELD VERIFY: INCONCLUSIVE status={status_code}", flush=True)
